...
uint8_t rowRepeat[IMAGE_HEIGHT];
uint8_t colRepeat[IMAGE_WIDTH];
uint16_t lineBuffer[SCREEN_WIDTH];
uint16_t gradientPalette[256];

bool overlayDirty = true;
float overlayDisplayedFPS = -1.0f;
...
static void setDebugMessage(const String &message) {
...
static void drawOverlay() {
  ...
  overlayDisplayedFPS = currentFPS;
  overlayDirty = false;
}

static void initializeColorPalette() {
  for (uint16_t i = 0; i < 256; ++i) {
    ...
    gradientPalette[i] = display.color565(static_cast<uint8_t>(r), static_cast<uint8_t>(g),
                                          static_cast<uint8_t>(b));
  }
}

static void initializeUpscaleTables() {
  uint16_t assigned = 0;
  for (uint16_t row = 0; row < IMAGE_HEIGHT; ++row) {
    uint16_t end = ((row + 1) * SCREEN_HEIGHT) / IMAGE_HEIGHT;
    if (end <= assigned) {
      end = assigned + 1;
    }
    ...
    rowRepeat[row] = static_cast<uint8_t>(end - assigned);
    assigned = end;
  }

  assigned = 0;
  for (uint16_t col = 0; col < IMAGE_WIDTH; ++col) {
    uint16_t end = ((col + 1) * SCREEN_WIDTH) / IMAGE_WIDTH;
    if (end <= assigned) {
      end = assigned + 1;
    }
    ...
    colRepeat[col] = static_cast<uint8_t>(end - assigned);
    assigned = end;
  }
}

static inline uint16_t mapTemperatureToColor(float value, float minValue, float scale) {
  if (scale <= 0.0f) {
    return gradientPalette[128];
  }
  float normalized = (value - minValue) * scale + 0.5f;
  int32_t idx = static_cast<int32_t>(normalized);
  if (idx < 0) {
    idx = 0;
  } else if (idx > 255) {
    idx = 255;
...
static void updateFPS() {
  ...
  lastFrameMillis = now;
  if (overlayDisplayedFPS < 0.0f || currentFPS > overlayDisplayedFPS + 0.1f ||
      currentFPS < overlayDisplayedFPS - 0.1f) {
    overlayDirty = true;
  }
}

static void setStreamingMessage() {
  setDebugMessage(String("Stream ") + kI2CProfiles[currentProfileIndex].label);
}
...
static bool acquireFrame(float *buffer) {
  ...
  for (uint8_t attempt = 0; attempt < kMaxAttempts; ++attempt) {
    ...
    if (status == -1) {
      ...
      if (consecutiveI2CErrors >= 3 && currentProfileIndex + 1 < kProfileCount &&
        ...
        applyI2CProfile(currentProfileIndex + 1);
        setDebugMessage(String("I2C->") + kI2CProfiles[currentProfileIndex].label + " (" + status + ")");
      } else {
        setDebugMessage(String("MLX I2C(") + status + ")");
      }
    } else {
      consecutiveI2CErrors = 0;
      setDebugMessage(String("MLX ") + describeMlxError(status) + " (" + status + ")");
    }
  ...
  if (sawDataNotReady) {
    ...
    if (consecutiveDataNotReadyFailures >= 6 && currentProfileIndex + 1 < kProfileCount &&
        millis() - lastProfileChangeMillis > 500) {
      applyI2CProfile(currentProfileIndex + 1);
      setDebugMessage(String("Slow->") + kI2CProfiles[currentProfileIndex].label);
    }
  }

  if (!haveValidFrame) {
    if (millis() - startWait > 150 && debugMessage != "Waiting") {
      setDebugMessage("Waiting");
    }
  }
  return false;
}
...
void loop() {
  ...
  for (uint16_t srcRow = 0; srcRow < IMAGE_HEIGHT; ++srcRow) {
    const float *rowPtr = &frameBuffer[srcRow * IMAGE_WIDTH];
    uint16_t destIndex = 0;
    for (uint16_t srcCol = 0; srcCol < IMAGE_WIDTH; ++srcCol) {
      uint16_t repeats = colRepeat[srcCol];
      uint16_t color = mapTemperatureToColor(rowPtr[srcCol], minTemp, scale);
      for (uint16_t copy = 0; copy < repeats && destIndex < SCREEN_WIDTH; ++copy) {
        lineBuffer[destIndex++] = color;
      }
    }
    if (destIndex < SCREEN_WIDTH) {
      uint16_t fillColor = destIndex > 0 ? lineBuffer[destIndex - 1] : gradientPalette[128];
      while (destIndex < SCREEN_WIDTH) {
        lineBuffer[destIndex++] = fillColor;
      }
    }
    uint8_t rowCopies = rowRepeat[srcRow];
    for (uint8_t copy = 0; copy < rowCopies; ++copy) {
