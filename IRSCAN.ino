...
static inline uint16_t colorFromValue(float value, float scale, float offset) {
  int32_t scaled = static_cast<int32_t>(value * scale + offset + 0.5f);
  if (scaled < 0) {
    scaled = 0;
  } else if (scaled > 255) {
    scaled = 255;
  }
  return colorLUT[scaled];
}
...
void loop() {
  ...
  const float *tempPtr = frameBuffer + 1;
  const float *tempEnd = frameBuffer + (IMAGE_WIDTH * IMAGE_HEIGHT);
  for (; tempPtr < tempEnd; ++tempPtr) {
    float value = *tempPtr;
    if (value < minTemp) {
      minTemp = value;
    }
  ...
  float offset = 0.0f;
  if (maxTemp > minTemp) {
    scale = 255.0f / (maxTemp - minTemp);
    offset = -minTemp * scale;
  }
  ...
  for (uint16_t y = 0; y < SCREEN_HEIGHT; ++y) {
    ...
    const uint8_t *colIndex = colLUT;
    for (uint16_t x = 0; x < SCREEN_WIDTH; ++x, ++colIndex) {
      float value = rowPtr[*colIndex];
      lineBuffer[x] = colorFromValue(value, scale, offset);
    }
    display.writePixels(lineBuffer, SCREEN_WIDTH, true);
  }
  display.endWrite();
