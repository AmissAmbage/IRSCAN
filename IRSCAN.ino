...
// Older releases of the MLX90640 library do not expose every error constant.
#ifndef MLX90640_FRAME_DATA_ERROR
...
static bool acquireFrame() {
  ...
  for (uint8_t attempt = 0; attempt < kMaxAttempts; ++attempt) {
    ...
    if (status == MLX90640_FRAME_DATA_ERROR) {
      // Sensor has not completed the next subframe yet; give it a moment and retry.
